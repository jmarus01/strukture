...
Position MakePol(int koeff, int exp);
...
int AddUp(Position P1, Position P2);
...
Position MakePol(int koeff, int exp) {           // funkcija za sastavljanje polinoma
...
int ReadLine(Position head, char* buffer) {             // funkcija za citanje redaka
    ...
    while (strlen(buffer) > 0) {
        procitan_bb = sscanf(buffer, " %d  %d  %n ", &koeff, &exp, &brojac);

        if (procitan_bb == 2 && koeff != 0)
            InsertSorted(head, MakePol(koeff, exp));              // sortiranje za brze izvodenje
...
int InsertSorted(Position head, Position NewElement) {              // funkcija za sortiranje
    ...
    if (head->next != NULL && head->next->exp == NewElement->exp) {
        head->next->koeff += NewElement->koeff;
        n = head->koeff;
    ...
    // ako je novi element manjeg eksponenta od ostalih samo ga stavi na pocetak liste
        ...
        MakePol(NewElement->koeff, NewElement->exp);
...
int AddUp(Position p1, Position p2) {           // funkcija za zbrajanje
    p1 = p1->next;
    p2 = p2->next;
    Position temp;
    ...
    while (p1 != NULL && p2 != NULL) {
        if (p1->exp == p2->exp) {
            suma_koeficijent = p1->koeff + p2->koeff;
            printf("\t%dx^%d\n", suma_koeficijent, p1->exp);
            p1 = p1->next;
            p2 = p2->next;
    ...
    while (temp != NULL) {
        printf(" ovaj:\t%dx^%d", temp->koeff, temp->exp);
        temp = temp->next;
    }
    return 0;
...
int Mutltiply(Position p1, Position p2) {                           // funkcija za mnozenje
    int umnozak_koeficijenata = 1, zbroj_eksponenata = 0;
    p1 = p1->next;
    p2 = p2->next;
    if (p1 == NULL) { printf("null je"); }
    while (p1 != NULL) {
        umnozak_koeficijenata = umnozak_koeficijenata * p1->koeff;
        zbroj_eksponenata = zbroj_eksponenata + p1->exp;
        printf("trenutno stanje : %dx^%d\n", umnozak_koeficijenata, zbroj_eksponenata);
        p1 = p1->next;

    }
    while (p2 != NULL) {
        umnozak_koeficijenata = umnozak_koeficijenata * p2->koeff;
        zbroj_eksponenata = zbroj_eksponenata + p2->exp;
        printf("trenutno stanje : %dx^%d\n", umnozak_koeficijenata, zbroj_eksponenata);
        p2 = p2->next;
    }
...
int main() {
    ...
    Poly head1 = { .koeff = 0 , .exp = 0, .next = NULL };
    Poly head2 = { .koeff = 0, .exp = 0, .next = NULL };

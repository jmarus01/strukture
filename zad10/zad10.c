...
int AddCountry(char* NameOfCountry, char* file, Position P)         //DODAJ DRZAVU
{
    Position q = NULL;
    Position prev = NULL;
    Tree c = NULL;
...
int AddCity(char* file, Tree S)             //DODAJ GRAD
{
    FILE* f = NULL;
    char name[50];
    int residents = 0;
...
Tree EnterCity(char* name, int residents, Tree S)           //UNOS GRAD
{
    Tree q = NULL;
    if (S == NULL)
    {
...
int PrintCountries(Position P)              //ISPISI DRZAVU
{
    printf("COUNTRIES:\n");
    while (P != NULL)
    {
...
int PrintCities(Tree S)             //ISPISI GRAD
{
    if (S == NULL)
        return 0;
    PrintCities(S->L);
...
Position SearchCountry(Position P, char* name)          //TRAZI DRZAVU
{
...
int SearchCity(Tree S, int searched_number)         //TRAZI GRAD
{
    if (S == NULL) {
        return 0;
    }

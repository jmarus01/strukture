...
Tree AddCountry(Tree S, char* NameOfCountry, char* NameOfFile)      //DODAJ DRZAVU
{
    Tree q = NULL;
    Position c = NULL;
...
int AddCity(Position P, char* file)             //DODAJ GRAD
{
    char name[50];
    int population = 0;
...
Tree SearchCountry(Tree S, char* NameOfCountry)         //TRAZI DRZAVU
{
    if (S == NULL)
        return NULL;
...
int Search(Tree S, char* NameOfCountry, int residents)      //TRAZI
{
    Position q = NULL;
    Tree R = NULL;
...
int PrintCities(Position P)     //ISPISI GRAD
{
    while (P != NULL)
    {
...
int PrintCountries(Tree S)      //ISPISI DRZAVU
{
    if (S == NULL)
        return 0;
